...
A Golang toolkit to extract Sudoku grids from screenshots and solve them via a feature-rich, interactive GUI. Built with the Fyne framework, it offers a seamless experience across Desktop (Windows/Linux) and Mobile (Android).

## Core Features

### 🤖 AI-Powered Extraction
- **Image-to-Grid**: Uses Gemini CLI (Gemini 3 Pro/Flash) to scan Sudoku screenshots and convert them into digital grids with near-perfect accuracy.
...
- **Use this prompt**: ACT AS A SUDOKU SCANNER. Extract the 9x9 grid from this image. Return ONLY a JSON object: {"grid": [[...],[...],...]}. Zero (0) for empty cells. CRITICAL: Every row MUST have exactly 9 numbers. NO MARKDOWN. NO CHAT.
...
- **Raise Hand (Single Hint)**: Stuck on a move? The "Raise Hand" feature identifies the next logical step and places a single digit. 
  - **Visual Cue**: Hinted cells flash light yellow and the digit is permanently marked in **Pink** for easy tracking.
...
- **Digit Scanning & Note Tracking**: 
  - Tapping any filled cell highlights all occurrences of that digit in **Vibrant Light Green**.
...
- **Multi-Platform Consistency**: 
  ...
  - **Android**: Specialized file management to bypass SAF limitations, ensuring reliable Save/Load workflows.
...
## Usage
Run with an image to extract immediately:
```bash
./SudokuHelper puzzle.jpg
```
Or use the manual scaling option:
```bash
